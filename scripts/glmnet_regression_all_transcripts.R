...
pdf(file="boxplot_coeff_glmnet_regression_all_transcripts_exclmonoexonic_ggplot2.pdf", width=9, height = 8)
par(mar=c(11,5,5,2))
ggplot(df_coeff_new, aes(x=reorder(name, value, FUN=median),y=value))+
  ...
  labs(title="Regularized regression all transcripts (excl monoex)")
dev.off()
...
pdf(file="scatterplot_glmnet_regression_all_transcripts_exclmonoex_ggplot2.pdf", width=9, height = 7)
ggplot(best_prediction_cc, aes(x=best_prediction_cc[,1],y=best_prediction_cc[,2])) +
  geom_point(size=3, col='gray55') +
  ...
  labs(title="Regularized linear regression all transcripts (excl monoex)")
dev.off()
...
colnames(df_coeff_new)=c("b", "name","value")
pdf(file="boxplot_coeff_glmnet_regression_all_transcripts_ggplot2.pdf", width=9, height = 8)
par(mar=c(11,5,5,2))
ggplot(df_coeff_new, aes(x=reorder(name, value, FUN=median),y=value))+
  geom_boxplot(fill="gray55", colour="black", notch = FALSE, lwd=0.5)+
  #geom_jitter(color="black", size=0.5, alpha=0.9)+
  ...
  scale_x_discrete(name="Features")+theme_bw()+
  theme(axis.text.y = element_text(size = 17, face="bold"), axis.text.x = element_text(angle = 90, size =17, vjust = 0.5, hjust=1, face="bold"), plot.title=element_text(size=20, face="bold"), axis.title=element_text(size=17, face="bold"))+
  labs(title="Regularized regression all transcripts")
dev.off()
...
pdf(file="scatterplot_glmnet_regression_all_transcripts_ggplot2.pdf", width=10, height = 6)
ggplot(best_prediction_cc, aes(x=best_prediction_cc[,1],y=best_prediction_cc[,2])) +
  geom_point(size=3, col='gray55') +
  ...
  theme_bw()+
  theme(axis.text.y = element_text(size = 20), axis.text.x = element_text(size =20), plot.title=element_text(size=22, face="bold"), axis.title=element_text(size=17, face="bold"))+
  labs(title="Regularized linear regression all transcripts")
dev.off()

print(cor(l[[ii]][,1],l[[ii]][,2]))
write.table(best_prediction_cc, 'predictions_alltranscripts', sep = ',', quote=F, row.names=F)

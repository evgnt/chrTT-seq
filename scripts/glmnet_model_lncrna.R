...
#t$SE_mean[round(t$exon_density*t$txt_length/1000.0) == 1] <-runif(1,0.001,0.01)
#t$SE_min[round(t$exon_density*t$txt_length/1000.0) == 1] <-runif(1,0.001,0.01)
# let's plot distribution of SE_min and SE_mean for fast and slow as a boxplot
#xx = t$SE_mean[round(t$exon_density*t$txt_length/1000.0) > 1 & t$id=="slow"]
#yy = t$SE_mean[round(t$exon_density*t$txt_length/1000.0) > 1 & t$id=="fast"]
...
ggplot(df_coeff_new, aes(x=reorder(name, value, FUN=median),y=value))+
  geom_boxplot(fill="indianred1", colour="black", notch = TRUE, lwd=1.0)+
